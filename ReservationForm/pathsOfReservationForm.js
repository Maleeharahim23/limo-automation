...
class ReservationFormPage {
    constructor(page) {
        this.page = page;
        this.ipageID = "formpage"
        this.pickupCityID = "pickupCity"
        this.carID = "/html/body/main/div[2]/div[2]/form/div[1]/div[1]/div/div[1]"
    ...
    async switchToReservationIpage() {
        await this.page.waitForSelector(`ipage#${this.ipageID}`, {timeout: 10000});
        this.page = this.page.pageLocator(`ipage#${this.ipageID}`);
        await this.page.waitForTimeout(300);
    }

    async selectServiceType(serviceTypeIndex) {
        const sel = this.page.locator(`xpath=${this.serviceTypeDropdownXPath}`);
    ...
    async selectPickUpAirportDropdown2(index) {
        const dropdown = this.page.locator(`xpath=${this.pickUpAirportDropdown2XPath}`);
        await dropdown.waitFor({state: 'visible'});
        await dropdown.selectOption({index});
        await this.page.waitForTimeout(1000);
    }
    ...
    async selectReturnPickupAirport(index) {
        const dropdown = this.page.locator(`xpath=${this.returnPickupAirport}`);
        await dropdown.waitFor({state: 'visible'});
        await dropdown.selectOption({index});
        await this.page.waitForTimeout(1000);
    }
    ...
    async selectLeg1ForRoundTrip(index) {
        const dropdown = this.page.locator(`xpath=${this.legDropdownForRoundTripXPath}`);
        if (index === 0) {
    ...
    async selectPickupState(index) {
        const el = this.page.locator(`xpath=${this.pickupStateXPath}`);
        await el.waitFor({state: 'visible'});
        await el.selectOption({index: Number(index)});
    }

    async selectDropoffAirport() {
        const el = this.page.locator(`xpath=${this.dropOffAirportDropdownXPath}`);
        await el.waitFor({state: 'visible'});
        await el.selectOption({index: 1});
    }

    async selectDropoffAirport2() {
        const el = this.page.locator(`xpath=${this.dropOffAirportDropdownXPath2}`);
        await el.waitFor({state: 'visible'});
        await el.selectOption({index: 1});
    }

    async selectReturnDropOffAirport() {
        const el = this.page.locator(`xpath=${this.returnDropOffAirport}`);
        await el.waitFor({state: 'visible'});
        await el.selectOption({index: 1});
    }

    async enterReturnDropOffAddress2(returnDropOffAddress2Input) {
        const input = this.page.locator(`xpath=${this.returnDropOffAddress2}`);
        await input.fill(returnDropOffAddress2Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[4]/div[3]/div/div/div[2]/div/ul/li[1]`);
    ...
    async selectDropOffState(index) {
        const el = this.page.locator(`xpath=${this.dropOffStateXPath}`);
        await el.waitFor({state: 'visible'});
        await el.selectOption({index: Number(index)});
    }

    async clickNextBtn1() {
        const btn = this.page.locator(`xpath=${this.nextBtn1}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async selectCar() {
        const el = this.page.locator(`xpath=${this.carID}`);
        // const el = this.page.locator(`[id="${this.carID}"]`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async selectPickupDate() {
        const dateInput = this.page.locator(`[name="${this.pickUpDateName}"]`);
        await dateInput.waitFor({state: 'visible', timeout: 10000});
        await dateInput.click();

        const calendarPopup = this.page.locator('.react-datepicker');
        ...
        const dateCell = this.page.locator(`div[aria-label="${ariaLabel}"]`);
        await dateCell.waitFor({state: 'visible', timeout: 10000});
        await dateCell.click();
    }

    async selectReturnDate(daysAhead = 10) {
        const dateInput = this.page.locator(`xpath=${this.returnDateXPath}`);
        await dateInput.waitFor({state: 'visible', timeout: 10000});
        await dateInput.click();
        ...
        let dateCell = this.page.locator(`div[aria-label="${ariaLabel}"]`);

        // If date not visible, click next month until found
        const nextMonthBtn = this.page.locator('.react-datepicker__navigation--next');
        let attempts = 0;
        while (!(await dateCell.isVisible()) && attempts < 2) {
            await nextMonthBtn.click();
            await this.page.waitForTimeout(500); // ✅ use page, not page
            attempts++;
            dateCell = this.page.locator(`div[aria-label="${ariaLabel}"]`);
        }
    ...
    async selectReturnDate2(daysAhead = 10) {
        const dateInput = this.page.locator(`xpath=${this.returnDateXPath2}`);
        await dateInput.waitFor({state: 'visible', timeout: 10000});
        await dateInput.click();
    ...
    async enterReturnTime(timeInput) {
        const el = this.page.locator(`#${this.returnTimeID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(timeInput);
    }

    async enterPickupAddress(addr) {
        const el = this.page.locator(`xpath=${this.pickUpAddressXPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(addr);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div/div[2]/div[1]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async enterPickupAddress2(addr) {
        const el = this.page.locator(`xpath=${this.pickUpAddress2XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(addr);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div/div[3]/div[1]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async enterPickupAddress3(addr) {
        const el = this.page.locator(`xpath=${this.pickUpAddress3XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(addr);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div/div[2]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async enterDropOffAddress1(dropOffAddress1Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress1XPath}`);
        await input.fill(dropOffAddress1Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[2]/div[5]/div/div/div[2]/div/ul/li[2]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterDropOffAddress2(dropOffAddress2Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress2XPath}`);
        await input.fill(dropOffAddress2Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[2]/div[5]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterDropOffAddress3(dropOffAddress3Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress3XPath}`);
        await input.fill(dropOffAddress3Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[4]/div[1]/div/div[2]/div/ul/li[2]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterDropOffAddress4(dropOffAddress4Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress4XPath}`);
    ...
    async enterDropOffAddress5(dropOffAddress5Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress5XPath}`);
        await input.fill(dropOffAddress5Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div/div[2]/div[3]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterDropOffAddress6(dropOffAddress6Input) {
        const input = this.page.locator(`xpath=${this.dropOffAddress6XPath}`);
        await input.fill(dropOffAddress6Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div/div[3]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterReturnPickupAddress(returnPickupAddressInput) {
        const input = this.page.locator(`xpath=${this.returnPickUpAddress}`);
        await input.fill(returnPickupAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[5]/div[2]/div/div/div[2]/div/ul/li[2]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterReturnPickupAddress2(returnPickupAddress2Input) {
        const input = this.page.locator(`xpath=${this.returnPickUpAddress2}`);
        await input.fill(returnPickupAddress2Input);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[4]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
        await this.page.waitForTimeout(1000);
    }
    ...
    async clickStopBtn1() {
        const el = this.page.locator(`xpath=${this.stopBtn1XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress1(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress1XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[2]/div[4]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async clickStopBtn2() {
        const el = this.page.locator(`xpath=${this.stopBtn2XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress2(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress2XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[2]/div[7]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async clickUseSameStopsForLeg2CheckBox() {
        const el = this.page.locator(`#${this.useSameStopsForLeg2CheckBoxID}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async clickStopBtn3() {
        const el = this.page.locator(`xpath=${this.stopBtn3XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress3(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress3XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[5]/div[5]/div[2]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async clickStopBtn4() {
        const el = this.page.locator(`xpath=${this.stopBtn4XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress4(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress4XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[4]/div[3]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async clickStopBtn5() {
        const el = this.page.locator(`xpath=${this.stopBtn5XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress5(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress5XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[5]/div[6]/div[2]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async clickStopBtn6() {
        const el = this.page.locator(`xpath=${this.stopBtn6XPath}`);
        await el.waitFor({state: 'visible'});
        await el.click();
    }

    async enterStopAddress6(stopAddressInput) {
        const el = this.page.locator(`xpath=${this.stopAddress6XPath}`);
        await el.waitFor({state: 'visible'});
        await el.fill(stopAddressInput);
        const suggestion = this.page.locator(`xpath=/html/body/main/div[2]/div[2]/form/div[1]/div[4]/div[4]/div[2]/div[2]/div[1]/div[2]/div/div/div[2]/div/ul/li[1]`);
        await suggestion.waitFor({state: 'visible'});
        await suggestion.click();
    }

    async enterAirlineName(airlineName) {
        const el = this.page.locator(`#${this.airlineID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(airlineName);
    }

    async enterDropOffAirlineName(dropOffAirlineNameInput) {
        const el = this.page.locator(`[name="${this.dropOffAirlineName}"]`);
        await el.waitFor({state: 'visible'});
        await el.fill(dropOffAirlineNameInput);
    }

    async enterReturnAirlineName(returnAirlineNameInput) {
        const el = this.page.locator(`[name="${this.returnAirlineName}"]`);
        await el.waitFor({state: 'visible'});
        await el.fill(returnAirlineNameInput);
    }

    async enterName(name) {
        const el = this.page.locator(`#${this.nameID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(name);
    }

    async enterEmail(email) {
        const el = this.page.locator(`#${this.emailID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(email);
    }

    async enterPhoneNumber(phoneNumber) {
        const el = this.page.locator(`[name="${this.phoneNumberName}"]`);
        await el.waitFor({state: 'visible'});
        await el.fill(phoneNumber);
    }

    async clickNextBtn2() {
        const btn = this.page.locator(`xpath=${this.nextBtn2}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickConsentSmsCheckbox() {
        const btn = this.page.locator(`#${this.consentSmsCheckboxID}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickGetQuoteBtn1() {
        const btn = this.page.locator(`xpath=${this.getQuoteBtn1XPath}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickGetQuoteBtn2() {
        const btn = this.page.locator(`xpath=${this.getQuoteBtn2XPath}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickGetQuoteBtn3() {
        const btn = this.page.locator(`xpath=${this.getQuoteBtn3XPath}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickGetQuoteBtn4() {
        const btn = this.page.locator(`xpath=${this.getQuoteBtn4XPath}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async clickGetQuoteBtn5() {
        const btn = this.page.locator(`xpath=${this.getQuoteBtn5XPath}`);
        await btn.waitFor({state: 'visible'});
        await btn.click();
    }

    async enterNumberOfPassengers(numberOFPassengers) {
        const el = this.page.locator(`#${this.numberOFPassengersID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(numberOFPassengers);
    }

    async enterPassengerNames(passengerName) {
        const el = this.page.locator(`#${this.passengerNamesID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(passengerName);
    }

    async enterLugage(lugage) {
        const el = this.page.locator(`#${this.lugageID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(lugage);
    }

    async enterPakPhoneNumber(phoneNumberInput) {
        const dropdown = this.page.locator('.selected-flag');
        await dropdown.click();
        const searchInput = this.page.locator(`xpath=//input[@type='tel' or @type='text']`);
        await searchInput.waitFor({state: 'visible'});
        await searchInput.fill('Pakistan');
        const pakistanOption = this.page.locator(`xpath=//li[contains(@class, 'country') and contains(., 'Pakistan')]`);
        await pakistanOption.waitFor({state: 'visible'});
        await pakistanOption.click();
        const phoneInput = this.page.locator(`[name="${this.phoneNumberName}"]`);
    ...
    async enterAdditionalNotes(additionalNotesInput) {
        const el = this.page.locator(`#${this.additionalNotesID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(additionalNotesInput);
    }

    async enterPickUpTime(departureTimeInput) {
        const el = this.page.locator(`#${this.pickUpTimeID}`);
        await el.waitFor({state: 'visible'});
    ...
    async enterDeparturetime(departureTimeInput) {
        const el = this.page.locator(`#${this.departuretimeID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(departureTimeInput);
        await this.page.waitForTimeout(1000);
    }
    ...
    async enterDropOffDepartureTime(dropOffDepartureTimeInput) {
        const el = this.page.locator(`#${this.dropOffDepartureTimeID}`);
        await el.waitFor({state: 'visible'});
    ...
    async enterAirlineArrivalTime(airlineArrivalTimeInput) {
        const el = this.page.locator(`#${this.airlineArrivalTimeID}`);
        await el.waitFor({state: 'visible'});
    ...
    async enterAirlineDeparturetime(departureTimeInput) {
        const el = this.page.locator(`#${this.airlineDeparturetimeID}`);
        await el.waitFor({state: 'visible'});
        await el.fill(departureTimeInput);
        await this.page.waitForTimeout(1000); // replaces time.sleep(1)
    }
    ...
    async checkValidationErrorsForOneWayTrips() {
        try {
            ...
            await this.page.locator('div[aria-live="assertive"] p.text-red-500')
                .first()
                .waitFor({state: 'visible', timeout: 5000});

            const errors = await this.page.locator('div[aria-live="assertive"] p.text-red-500').allTextContents();
