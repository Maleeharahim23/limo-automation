...
class SendQuotes {
    ...
    async selectDropdownOption(dropdownLabel, optionText) {
        // Try to locate the dropdown using visible text (covers both <button> and custom divs)
        const dropdownButton = this.page.locator(`//*[normalize-space(text())='${dropdownLabel}']`);

        // Wait for dropdown trigger to appear and be visible
        await dropdownButton.waitFor({ state: 'visible', timeout: 20000 });
        await dropdownButton.click();

        // Give dropdown time to expand
        await this.page.waitForTimeout(1000);

        // Try multiple ways to locate the option
